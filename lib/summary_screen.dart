...
class SummaryScreen extends StatelessWidget{
  ...
  Widget _renderGoals(p) {
    ...
    if (counter > 0) {
      ...
      return
        Text("No goals.", style: TextStyle(fontSize: 16),);      
    }
  }
  ...
  Widget _renderShots(p) {
    ...
    if ((h > 0) | (a > 0)) {
      return
        RichText(
          text: TextSpan(
            ...
            children: <TextSpan>[
              TextSpan(text: "Shots: ", style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      return
        Text("No shots.", style: TextStyle(fontSize: 16),);    
    }
  }  
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              Padding(padding: const EdgeInsets.all(12.0),),
              Center(
                child: Text(summary.homeTeam + " (H) vs " + summary.awayTeam + " (A)", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22)),
              ),
              Center (
                child: Text(DateFormat('MM/dd').format(DateTime.now()), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22)),
              ),
              Padding(padding: const EdgeInsets.all(12.0),),
              Text("1st Period", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
              _renderShots(enumPeriodType.one),
              _renderSpace(enumPeriodType.one),
              _renderGoals(enumPeriodType.one),
              Padding(padding: const EdgeInsets.all(12.0),),
              Text("2nd Period", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
              _renderShots(enumPeriodType.two),
              _renderSpace(enumPeriodType.two),
              _renderGoals(enumPeriodType.two),
              Padding(padding: const EdgeInsets.all(12.0),),
              Text("3rd Period", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
              _renderShots(enumPeriodType.three),
              _renderSpace(enumPeriodType.three),
              _renderGoals(enumPeriodType.three),
              Padding(padding: const EdgeInsets.all(12.0),),
              Text("Overtime", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
              _renderShots(enumPeriodType.ot),
              _renderSpace(enumPeriodType.ot),
              _renderGoals(enumPeriodType.ot),
              Padding(padding: const EdgeInsets.all(12.0),),
              Text("Shootout", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
              _renderShots(enumPeriodType.so),
              _renderSpace(enumPeriodType.so),
              _renderGoals(enumPeriodType.so),
              Padding(padding: const EdgeInsets.all(12.0),),
              Center(
                child: FloatingActionButton(heroTag: "fab9", onPressed: () { Navigator.pop(context); }, backgroundColor: Colors.black, tooltip: "Back", mini: true, child: Icon(Icons.arrow_back)),
              ),
            ],
          ),
        )
